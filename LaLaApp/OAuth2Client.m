...
-(id)init {
    ...
    if (self = [super init]) {
        ...
        _baseUrl = @"";
        _tokenEndpoint = @"/protocol/openid-connect/token";
        _authorizationEndpoint = @"/protocol/openid-connect/auth";
        _endSessionEndpoint = @"/protocol/openid-connect/logout";
...
/*
    returning all  response values (complete response) as dictionary
 */
...
/*
    values per per response parameter
 */
...
/*
    Method that refreshes the accessToken using the refreshToken. Returns BOOL success
 */
-(BOOL)refreshToken
{
...
/*
    Communicates with server to exchange the authorization_code for accessToken
 */
...
/*
 returning uri string constructed with predefined urls parts for token endpoint
 */
...
/*
 returning uri string constructed with predefined urls parts for authorization endpoint
 */
...
/*
    returns the authorization html header string
 */
...
/*
    appends OAUTH2 parameters to the query string
 */
-(NSString *)appendOAuthRequestParameter:(OAuth2RequestParameter)parameter
{
    if ([self OAuthRequestParameterExists:parameter]) {
        return [NSString stringWithFormat:@"&%@=%@", [OAuth2Client requestParameterEnumToString:parameter], [[self getOAuthRequestParameter:parameter]stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]]];
    }
...
/*
    returning uri string constructed with predefined urls parts for authorization
 */
...
/*
    returns the built post data for sending to the token endpoint
 */
...
/*
 returns the built post data for sending to the token endpoint WITH refresh token for refreshing the access token
 */
...
/*
    called for both requesting new access token AND for refreshing the access token
 */
...
/*
    method for actually calling the token endpoint, must be called with built post data
 */
...
/*
 Method for ending the session of the token
 */
-(void)callEndSessionEndpoint{
    NSString *endSessionURLString = [NSString stringWithFormat:@"%@%@?id_token=%@",_baseUrl,_endSessionEndpoint,[responseData valueForKey:@"id_token"]];
    NSLog(@"end session endpoint url: %@",endSessionURLString);
//    [[[NSURLSession sharedSession] dataTaskWithURL:[NSURL URLWithString:endSessionURLString]] resume];
    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:endSessionURLString] options:@{} completionHandler:^(BOOL success) {
        NSLog(@"++success: %d",success);
    }];
}

//This method processes the response for an authorization code grant type
...
/*
 returns state of the response error header
 */
-(BOOL)didSucceed {
...
/*
    returns string type description of all request parameters
 */
...
{
    [aCoder encodeObject:requestParameters forKey:@"requestParameters"];
    [aCoder encodeObject:responseData forKey:@"responseData"];
    [aCoder encodeObject:_baseUrl forKey:@"baseUrl"];
    [aCoder encodeObject:_authorizationEndpoint forKey:@"authorizationEndpoint"];
    [aCoder encodeObject:_tokenEndpoint forKey:@"tokenEndpoint"];
    [aCoder encodeObject:_endSessionEndpoint forKey:@"endSessionEndpoint"];
    [aCoder encodeObject:_associatedRequest forKey:@"associatedRequest"];
    [aCoder encodeObject:[NSNumber numberWithInt:_requestType] forKey:@"requestType"];
}
...
-(instancetype)initWithCoder:(NSCoder *)aDecoder
{
    self = [super init];
    if (self) {
        requestParameters = [aDecoder decodeObjectForKey:@"requestParameters"];
        responseData = [aDecoder decodeObjectForKey:@"responseData"];
        _baseUrl = [aDecoder decodeObjectForKey:@"baseUrl"];
        _authorizationEndpoint = [aDecoder decodeObjectForKey:@"authorizationEndpoint"];
        _tokenEndpoint = [aDecoder decodeObjectForKey:@"tokenEndpoint"];
        _endSessionEndpoint = [aDecoder decodeObjectForKey:@"endSessionEndpoint"];
        _associatedRequest = [aDecoder decodeObjectForKey:@"associatedRequest"];
        _requestType = [[aDecoder decodeObjectForKey:@"requestType"] intValue];
    }
    return self;
}
